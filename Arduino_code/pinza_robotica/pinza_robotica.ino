...
#define P_FORZA A1
//Asse X Joypstick usato per pilotare la pinza (valori 0-1023 posizione a riposo 512)
#define P_SETPT_PINZA A7
//Asse y Joypstick usato per pilotare rotazione del polso (valori 0-1023 posizione a riposo 512)
...
int serial_inc_byte = 0;    //byte in ingresso da porta seriale pc usata per pilotaggio in alternativa al Joypstick
...
int Forza = 0;
int Joyp = 0;                //variabile per lettura joystick pinza
int Joyr = 0;                //variabile per lettura rotazione polso
int Mem_Joyp=0;
int Forza_Max= 255;
boolean Click_On =0;
boolean Mem_Click =0;
...
void loop()
{
 ...
 if (Serial.available() > 0) {
  ...
  // Qui abilito con il click del Joypsitck il mantenimento di una posizione specifica, quando il led rosso è acceso la posizione viene mantenuta
  ...
  if(Click_On==0){
    ...
    Joyp = analogRead(P_SETPT_PINZA); //Leggo la compressione della pinza
    Joyp = Joyp-OffSet_Joyp;
    if(Joyp>Forza_Max){Joyp=Forza_Max;}
    if(Joyp<-255){Joyp=-255;}
